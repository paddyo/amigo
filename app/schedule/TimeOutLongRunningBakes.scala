...
import schedule.TimeOutLongRunningBakes.{BakesRepo, PackerEC2Client}
...
class TimeOutLongRunningBakes private[schedule](
    stage: String,
    bakesRepo: BakesRepo,
    packerEC2Client: PackerEC2Client
)(implicit dynamo: Dynamo) extends ScheduledJob with Loggable {
  ...
  def timeOutLongRunningBakes(earliestStartedAt: DateTime): Unit = {
  ...
  override def scheduleAction(): Unit = timeOutLongRunningBakes(earliestStartedAt = DateTime.now.minusHours(1))
}
